...
def test_task_extraction(sample_text=None, save_output=True):
    ...
    if save_output:
        return save_results(sample_text, prompt, raw_response, tasks, "tests/results")
    return None
...
def main():
    ...
    if args.split:
        ...
        split_files = split_test_files(args.split, "tests/sample_inputs")
        if split_files:
            print("\nRunning tests on split files...")
            process_sample_files("tests/sample_inputs", "tests/results")
    elif args.dir:
        process_sample_files(args.dir, "tests/results")
    elif args.file:
        try:
